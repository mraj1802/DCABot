...
import { SiEthereum, SiSololearn, SiBitcoinsv } from "react-icons/si";
...
const CreateDcaBot = () => {
  const pair = [
    ...
    },
    {
      value: "SOL/USDT",
      label: "SOL/USDT",
      icon: <SiSololearn className="text-red-600 text-lg" />,
    },
  ];
  ...
  const initialFormData = {
    botName: "",
    pair: "" || bot["pairs"],
    baseOrder: "" || bot["baseOrderSize"],
    safetyOrd: "" || bot["safetyOrderSize"],
    maxSafetyOrd: "" || bot["maxSafetyOrder"],
    priceDeviation: "" || bot["safetyOrderDeviation"],
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let Data = { ...formData, pair: selectedPair.value };
    const res = await axios.post(
      "http://localhost:8080/api/bot/orderlist",
      Data
    );
  ...
  const handleCreateBot = async () => {
    try {
      let data = {
        config: { ...formData, pair: selectedPair.value },
      ...
      console.log("error in handlecreatebot.", error);
    }
  };
