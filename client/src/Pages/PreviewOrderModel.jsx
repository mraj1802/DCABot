...
const PreviewOrderModel = ({
  ...
  const deviationValues = calculateData.map((entry) =>
    parseFloat(entry.deviation)
  );
              ...
              <table className="my-4 w-full border-collapse border border-gray-300 bg-[#1f2937] text-white">
                <thead>
                  <tr>
                    ...
                    <th className="border border-gray-300 px-1 py-1">Filled</th>
                  </tr>
                </thead>
                <tbody>
                  {calculateData.map((dataEntry, index) => {
                    if (
                      deviationExceeded &&
                      parseFloat(dataEntry.deviation) >= 100
                    ) {
                      return null;
                    }
                    return (
                      <tr key={`data${index}`}>
                        ...
                        <td className="border border-gray-300 px-4 py-2">
                          {dataEntry.filled === 0 ? "Waiting" : "Filled"}
                        </td>
                      </tr>
                    );
                  })}
