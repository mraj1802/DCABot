...
const PreviewOrderModel = ({ isOpen, onClose, submittedData ,onModify}) => {
    ...
    const calculateaverge = (prices) => {
      ...
      return isNaN(averages[averages.length - 1]) ? '0.00' : averages[averages.length - 1].toFixed(2);
    };
    ...
    const calculatesumqty = (qtys,index,baseorder,safetyorder,price) =>{
      ...
      for (let i = 2; i <= index; i++) {
        ...
        const previousadd = parseFloat((parseFloat(baseorder) / price)+(parseFloat(safetyorder) / price)).toFixed(5);
        sum += i === 2 ? parseFloat(parseFloat(numericQty) + parseFloat(previousadd)) : parseFloat(numericQty);
      }
      return sum;
        ...
        {isOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="bg-white p-8 rounded-lg relative h-[60vh] overflow-y-auto">
              <h2 className="text-2xl font-bold mb-4 text-center">Preview Order</h2>
                <button
                  onClick={onClose}
            ...
            <thead>
              <tr>
                ...
                <th className="border border-gray-300 px-1 py-1">Deviation</th>
                <th className="border border-gray-300 px-1 py-1">Price</th>
                <th className="border border-gray-300 px-1 py-1">Average</th>
                ...
                <th className="border border-gray-300 px-1 py-1">Sum(Qty)</th>
                <th className="border border-gray-300 px-1 py-1">Sum($)</th>
                <th className="border border-gray-300 px-1 py-1">Type</th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: submittedData.maxsafetyorder }, (_, index) => {
  
                const Deviation = calculateDeviation(submittedData.safetyorderdeviation, index);
                const baseprice = ethPrice;
                const price = calculatePrice(Deviation, baseprice);
                prices.push(price);
                const average = calculateaverge(prices) ;
                const target = calculatetarget(average,submittedData.targetprofit);
                const amount = calculateamount(submittedData.safetyordersize,submittedData.safetyordervolume,index);
                const qty = calculateqty(price,amount);
                amounts.push(amount);
                qtys.push(qty);
  
                const sumamount = calculatesumamount(amounts,index,submittedData.baseordersize,submittedData.safetyordersize);
                const sumqty = calculatesumqty(qtys,index,submittedData.baseordersize,submittedData.safetyordersize,price);
                ...
                return (
                  <tr key={`safetyorder_${index}`}>
                    ...
                    <td className="border border-gray-300 px-4 py-2"> ${parseFloat(target).toFixed(2)}</td>
                    <td className="border border-gray-300 px-4 py-2">
                    ${index === 0 ? (parseFloat(submittedData.baseordersize) / price).toFixed(5) : index === 1 ? (parseFloat(submittedData.safetyordersize) / price).toFixed(5) : parseFloat(qty).toFixed(5)}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                    ${index === 0 ? parseFloat(submittedData.baseordersize).toFixed(2) : index === 1 ? parseFloat(submittedData.safetyordersize).toFixed(2) : parseFloat(amount).toFixed(2)}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                    ${index === 0 ? (parseFloat(submittedData.baseordersize) / price).toFixed(5) : index === 1 ? parseFloat((parseFloat(submittedData.baseordersize) / price)+(parseFloat(submittedData.safetyordersize) / price)).toFixed(5) : parseFloat(sumqty).toFixed(5)}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                    ${index === 0 ? parseFloat(submittedData.baseordersize).toFixed(2) : index === 1 ? parseFloat(parseFloat(submittedData.baseordersize) + parseFloat(submittedData.safetyordersize)).toFixed(2) : parseFloat(sumamount).toFixed(2)}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">{submittedData.ordertype}</td>
                  </tr>
                );
              })}
            </tbody>
